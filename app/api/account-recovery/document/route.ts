    ...
    const uploadResult = await new Promise<any>((resolve, reject) => {
      ...
      const stream = cloudinary.uploader.upload_stream(
        ...
        (error: unknown, result: any) => {
          if (error) {
            const e: any = error
            const message = e?.message || e?.error?.message || (typeof e === "string" ? e : "Ошибка Cloudinary")
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!publicId.startsWith(`${FOLDER}/`)) {
      return NextResponse.json({ error: "Нельзя удалить файл вне папки mz/recovery" }, { status: 400 })
    }
    ...
    return NextResponse.json({ error: error?.message || "Ошибка при удалении" }, { status: 500 })
  }
}
