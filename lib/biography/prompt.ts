...
export function buildBiographyValidationPrompt(params: {
  ...
  return [
    ...
    {
      role: "user" as const,
      content: `ДАТА ДЛЯ ПРОВЕРКИ (ISO, дата написания биографии или сегодня): ${currentDateISO}
...
- Рассчитай возраст на основе ДАТЫ ДЛЯ ПРОВЕРКИ (это дата написания биографии, а не обязательно сегодняшняя дата).
  Формула:
  Возраст = Год написания биографии - Год рождения
  Если день рождения в году написания биографии ещё не наступил → вычесть 1 год
- Сравнение: difference = |statedAge - calculatedAge|.
- Если difference <= 1 → возраст валиден.
- Если difference > 1 → возраст невалиден.
- Возраст должен быть реалистичным: 18-80 лет.

2) Первое лицо:
- Пункты 10, 11, 12 ОБЯЗАТЕЛЬНО должны быть от первого лица, когда речь идет о самом персонаже.
- Если персонаж говорит о себе в третьем лице ("он", имя персонажа вместо "я" и т.п.) — это критическая ошибка.
 - Правило строгое: любые явные маркеры третьего лица в пунктах 10-12, когда речь идет о самом персонаже = status=error.
- Третье лицо допускается, когда персонаж говорит о других людях.
